...
import { createVariableService, getAllVariables, getVariableService } from './variables.service';
...
export const getAllVariableController = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const createVariableController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!name || !value || typeof name !== 'string' || typeof value !== 'string') {
            res.status(400).json({ message: 'Name and Value must be valid strings' });
            return;
        }
